...
{
  tensao = tensao +10;
  corrente = corrente + 11;
  leituraLDR = leituraLDR + 12;
  
 // Serial.println(RetornarHorario());
  Serial.println(LerTENSAOSensor());
  
  EnviarJson(tensao, corrente, leituraLDR); // Enviar Json Serviço
  GravarHorario(tensao, corrente, leituraLDR, RetornarHorario(), RetornarData(),UniqueId(), 1); // Gravar Hora Tabela
 }
...
void GravarHorario(float tensao, float corrente, float luminosidade, String horario, String data_atual, String UniqueId, int num)
{
   if (horario.compareTo("00:00:00") == 0)
  {
    GravarDadosMySql(tensao, corrente, luminosidade, horario, data_atual, UniqueId, num);
  }
  else if (horario.compareTo("01:00:00") == 0)
  {
    GravarDadosMySql(tensao, corrente, luminosidade, horario, data_atual, UniqueId, num);
  }
  else if (horario.compareTo("02:00:00") == 0)
  {
    GravarDadosMySql(tensao, corrente, luminosidade, horario, data_atual, UniqueId, num);
  }
  else if (horario.compareTo("03:00:00") == 0)
  {
    GravarDadosMySql(tensao, corrente, luminosidade, horario, data_atual,UniqueId, num);
  }
  else if (horario.compareTo("05:00:00") == 0)
  {
    GravarDadosMySql(tensao, corrente, luminosidade, horario, data_atual, UniqueId, num);
  }
  else if (horario.compareTo("06:00:00") == 0)
  {
    GravarDadosMySql(tensao, corrente, luminosidade, horario, data_atual, UniqueId, num);
  }
  ...
  else if (horario.compareTo("19:00:00") == 0)
  {
    GravarDadosMySql(tensao, corrente, luminosidade, horario, data_atual, UniqueId, num);
  }
  else if (horario.compareTo("20:00:00") == 0 )
  {
    GravarDadosMySql(tensao, corrente, luminosidade, horario, data_atual, UniqueId, num);
  }
  else if (horario.compareTo("21:00:00") == 0)
  {
    GravarDadosMySql(tensao, corrente, luminosidade, horario, data_atual, UniqueId, num);
  }
  else if (horario.compareTo("22:00:00") == 0)
  {
    GravarDadosMySql(tensao, corrente, luminosidade, horario, data_atual, UniqueId, num);
  }
  else if (horario.compareTo("23:00:00") == 0)
  {
    GravarDadosMySql(tensao, corrente, luminosidade, horario, data_atual, UniqueId, num);
  } 
}
...
void GravarDadosMySql(float tensao, float corrente, float luminosidade, String horario, String data_atual, String token, int flag)
{ 
  delay(1000);
  const int httpPort = 80;
  WiFiClient client2;
...
float LerLDRSensor()
{
  ...
  leituraLDR = map(valorpot, 0, 255, 0, 255); 
